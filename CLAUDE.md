...
| MOD-02b | `green_button.emulator` | **Deployed v2.4.7** | `green_button_app_v2_4_0.jsx` + `green_button_emulator.html` |
...
| MOD-06 | `island_dispatch` | **Deployed v0.4.137** (browser UI + Python prototype) | `island_dispatch_app_v0.1.0.jsx` + `island_dispatch.html` + `mod06_island_dispatch_v11.py` |
...
**Reference specs exist for:** MOD-02b (`MOD-02b_reference_spec.md`), MOD-06 (`MOD-06_island_dispatch_reference_spec_v0.4.68.md` — spec at v0.4.68, tool now at v0.4.137; spec is stale and needs update), MOD-09 (`MOD-09_tracker_analyzer_reference_spec.md`)
...
Off-grid systems are sized using **two criteria** (as of v0.4.116+):

1. **Criterion 1 — 3-day no-solar test:** Stationary battery alone (no PV, no EV) must power critical loads for 72 continuous hours. This is the Title 24 §150.1-C code compliance check.
2. **Criterion 2 — Full-year annual simulation:** A complete 8,760-hour simulation from Jan 1 must produce zero unserved load hours. This test subsumes the worst 10-day window — surviving the full year guarantees the worst window is covered. Systems sized purely on the worst window may still shed load during other extended low-solar stretches in the year; the annual simulation catches those.

The NSRDB JSON (v2.2+) stores **1,800 hours per cell** (75 days = 60 spinup + 10 WW + 5 post-window display days); the post-window hours are used only for chart display. The NSRDB stress window is pre-computed per cell and selected by haversine nearest-cell lookup for any customer lat/lon. `annualScale = 365 / coreDays` uses `coreDays = 70` where needed for extrapolation, but actual annual criterion uses 8760-hour simulation counts directly.
...
**Two filename versioning patterns are in use — choose one per module and stick with it:**

- **Rename-on-bump** (default for new modules): JSX filename carries the version, e.g. `rate_engine_app_v1.4.0.jsx` → `rate_engine_app_v1.5.0.jsx`. The HTML wrapper `<script src>` filename changes with every bump. Used by MOD-04 and MOD-09.
- **Stable filename** (used by MOD-06): The JSX filename never changes (`island_dispatch_app_v0.1.0.jsx`). All version tracking is internal — file header comment, on-screen UI string, and HTML wrapper `<script src>?v=` parameter. GitHub can diff the file across versions without dealing with renames. Choose this pattern when a module has frequent point-version bumps (dozens of iterations).

When bumping a version:
- **If rename-on-bump:** rename the JSX file; update the HTML wrapper `<script src>` filename and `?v=` parameter
- **If stable filename:** only the HTML wrapper `?v=` parameter changes (filename stays the same); verify the old `?v=` value is replaced
- In both patterns: update the HTML wrapper `<!-- Version: -->` comment and `<!-- Updated: -->` comment
...
**Method A — Git clone (used in active Claude Code sessions)**
A git clone of the repository lives at `/tmp/cce-solar-tools`. After editing files locally, copy them to the clone and push:
```bash
cp ~/Downloads/Project\ Files/island_dispatch_app_v0.1.0.jsx /tmp/cce-solar-tools/
cp ~/Downloads/Project\ Files/island_dispatch.html /tmp/cce-solar-tools/
cd /tmp/cce-solar-tools
git add island_dispatch_app_v0.1.0.jsx island_dispatch.html
git commit -m "MOD-06 vX.Y.Z — one-line description

Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>"
git push
```
The clone may not persist between sessions (it lives in `/tmp`). If missing: `git clone https://github.com/josetorre-bueno/cce-solar-tools.git /tmp/cce-solar-tools`

**Method B — Web interface drag-and-drop (fallback / manual)**
Navigate to `https://github.com/josetorre-bueno/cce-solar-tools`, drag files onto the file list, write a commit message, and click "Commit changes". No local git required.
...
*MOD-02b green_button.emulator (v2.4.7) — Help manual updated: new sections for Annual kWh Override and Per-Month Adjustment, and Summary section notes interactive monthly chart.*
*MOD-02b green_button.emulator (v2.4.6) — Three fixes: (1) Spinner applies immediately; typed values apply only on Enter/blur. Distinguished via isTypingRef: keydown sets it true before onChange for keypresses; spinner clicks fire onChange with no preceding keydown. (2) applyMonthAdj changed from smooth centre-to-centre interpolation to flat per-month multiplication — interpMonthMult was bleeding changes into months ±3 and beyond through interval-level blending, violating the spec. (3) Snapshot (startMults) is immutable during a spinner session; each click computes pct from the bar-click baseline so adjacent months track exactly half the total movement.*
*MOD-02b green_button.emulator (v2.4.5) — Per-month spreading fix attempt (superseded by v2.4.6): snapshot approach introduced but smooth-interpolation bleed and debounce timing still imperfect.*
*MOD-02b green_button.emulator (v2.4.4) — Live spinner feedback: monthMult update moved from onBlur into onChange so spinner clicks and live typing both update the bar chart without requiring Enter/blur.*
*MOD-02b green_button.emulator (v2.4.3) — Fix Reset compounding on repeated clicks. setSummary _scaleFactor to 1.0 immediately on reset so subsequent clicks see sf === 1.0 and skip the division. Previously each click divided the already-divided intervals by the old scale factor again.*
*MOD-02b green_button.emulator (v2.4.2) — Per-month kWh adjustment: click ✏ Edit near the Monthly kWh chart, click a month bar to select it, use ↑↓ arrow keys for ±1% steps (adjacent months ±0.5%, next-over ±0.25%), or type an exact kWh value + Enter. Smooth centre-to-centre interpolation (flat multiplication from v2.4.6). Annual total floats with adjustments. Two resets: Reset Months clears month multipliers; Reset in the annual override banner restores the pre-override model estimate. All output files rebuild automatically (debounced 400ms for CSV/XML).*
*MOD-02b green_button.emulator (v2.4.1) — Annual kWh override: optional Target Annual kWh field. If set, all 35,040 intervals scale proportionally so monthly kWh, monthly peak kW, and all output files match the specified total. Load shape preserved. Green banner in summary confirms override; Reset button undoes it in-place without re-running the simulation. baseIntervalsRef stores post-override intervals for live month editing.*
*MOD-06 browser UI (v0.4.137) — BATTERY_LIBRARY fixedCost values updated to current Enphase 10C and Tesla Powerwall 3 pricing (20 entries, 1x–10x each family). localStorage.clear() + page reload required to pick up new defaults if batteryCosts was previously cached.*
...
*MOD-06 browser UI (v0.4.130) — Two fixes: (1) Annual chart P1/P2 panel widths now align exactly — `afterFit(s) { s.width = ANN_Y_W; }` (ANN_Y_W = 54px) applied to all 4 y-scales (battery P1, battery P2, generator P1, generator P2), forcing identical left-axis widths so kW and kWh panels line up for hour-level comparison. (2) EV SOC now always shown in center-of-window panel even when away — replaces "Away" text with actual kWh + % + italic "· away" tag, making trip energy deductions and destination charging visible in the values panel.*
*MOD-06 browser UI (v0.4.131) — Fix EV SOC jumping up on trip return. Root cause: full roundTripKwh deducted at hr=18 return, but solar charging in the same timestep could more than offset it, producing apparent SOC increase. Fix: split deduction — one-way energy deducted at departure (hr=7) so trace shows EV leaving home with less charge; only the return-leg one-way deducted at hr=18. Also fixes l2_paid double-deduction: `arrivalAtWork = evE[i]` used directly (outbound energy already deducted at hr=7), no second subtraction.*
*MOD-06 browser UI (v0.4.136) — Three help manual corrections. (1) EV section expanded: per-field descriptions for all EV inputs; destination charging types (None / Free L2 / Paid L2) with model behavior for each; DCFC stop count fields (planned per-EV, en-route fleet max, emergency fleet max) with explanation that emergency DCFC excess is a rejection criterion for system sizing; charge targets (DCFC target 80%, dest L2 target 95%) with rationale; EV topologies section updated to reference actual UI field values. (2) Pass/fail criteria corrected from 3 to 2: C1 = 3-day no-solar Title 24 test (stationary battery only), C2 = full-year 8,760-hour annual simulation (zero unserved load). The old three-criterion system was superseded in v0.4.116. (3) Arrow key navigation note: keys only work when keyboard focus is inside the chart control area — user must click the range slider (or anywhere between the ← → buttons) before using arrow keys.*
*MOD-06 browser UI (v0.4.135) — Fix help text typo: PV sizes described as "DC array capacities" instead of "AC capacities". (AC output is what PVWatts reports; the input sweep sizes are DC nameplate.)*
*MOD-06 browser UI (v0.4.134) — (1) Help modal: `?` button in top bar opens a scrollable user manual covering tool overview, input fields, pass/fail criteria, annual dispatch chart navigation (click overview strip, scroll wheel zoom, ← → pan, range slider, arrow keys ±1 hr), complete color-coding guide for P1 power panel (solar fill, olive curtailment, dark-red load, orange generator, red unserved, teal en-route DCFC markers, red emergency DCFC markers) and P2 SOC panel (amber/green/teal EV stacks, dark-blue battery, dashed min-SOC reference, colored away shading), center-of-window delta annotations (↑ solar / ↑ gen / ↑ bat→EV / ↓ departed / etc.), EV topologies A/B/C/V2G, and usage tips. (2) `handleResetDefaults` now clears climate zone and floor area (cfa) to blank instead of resetting to hardcoded Sanchez-house values (10 / 1,626 sqft). Battery costs and product prices are unaffected by reset.*
*MOD-06 browser UI (v0.4.133) — Two display fixes. (1) Road trip DCFC trace alignment: at hr=7 departure with DCFC, the trace now shows the post-DCFC SOC (before the one-way trip deduction) at the departure hour, so the teal DCFC marker on the P1 chart aligns with the visible SOC jump in the P2 chart. The trip deduction is then visible at hr=8 when the EV first appears as "away". Implemented via `dcfcTracePatch[]` array in `simulatePeriod` that overrides `traceEvE[i][h]` and `traceEvAway[i][h]` at the road trip departure hour, then is cleared. (2) Per-EV colored away-state shading in P2 charts: `awayDs` (single combined boolean) replaced by `perEvAwayDs[]` (per-EV boolean arrays). Both `annBatEvAwayPlugin` and `annGenEvAwayPlugin` now loop over per-EV arrays and draw colored bands (amber/green/teal/red matching each EV's chart color) instead of a single grey band, making it immediately clear which specific EV is away.*
*MOD-06 browser UI (v0.4.132) — Add hour-to-hour Δ with source labels to center-of-window panel. Both battery column and generator column now show italic annotations: battery SOC shows `+/−N.N kWh ↑ solar / ↑ gen / ↓ load/EV`; each EV shows `+/−N.N ↑ solar / ↑ bat→EV / ↑ gen→EV / ↓ departed / ↓ return trip / ↓ trip + ↑ bat→EV / ↓ V2H/drive`. Makes prio-0 battery→EV emergency charging (fires at night when EV returns below transport minimum) immediately visible — the "where did the energy come from?" question answered by "↑ bat→EV" annotation.*
