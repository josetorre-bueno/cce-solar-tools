...
// Requires: rate_engine_v1.2.0.js loaded before this file (provides window.MOD04)
//
...
function App() {
  ...
  if (typeof MOD04 === 'undefined') {
    return (
      <div style={{ padding: 32, color: C.red, fontFamily: 'monospace', fontSize: 13 }}>
        Error: MOD04 not loaded. Verify that rate_engine_v1.2.0.js is included in the HTML
  ...
  const rates = useMemo(() => {
    ...
    if (customerType === 'residential_care') {
      return all.filter(r => r.careAvailable);
    }
    return all;
  }, [customerType]);
  ...
  const handleRateChange = useCallback(key => {
    setRateKey(key);
    const r = MOD04.RESIDENTIAL_RATES[key];
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '24px 24px 48px' }}>
        ...
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 20 }}>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`,
            ...
            {customerType === 'commercial' && rates.length === 0 && (
              <div style={{ fontSize: 11, color: C.orange, marginBottom: 14, lineHeight: 1.5 }}>
                Commercial rates (TOU-A, AL-TOU, AL-TOU-2) coming soon.
                Residential rates are available now.
              </div>
            )}

            {/* Rate note */}
            {MOD04.RESIDENTIAL_RATES[rateKey] && (
              <div style={{ fontSize: 11, color: C.muted, marginBottom: 14,
                            lineHeight: 1.5 }}>
                {MOD04.RESIDENTIAL_RATES[rateKey].rateNote}
              </div>
            )}
...
function BillResults({ results, rateKey }) {
  const { bill } = results;
  const chartData = useMemo(() => makeBillOnlyChartData(bill.monthly), [bill]);
  ...
  return (
    <div>
      ...
      {/* Monthly table */}
      <MonthlyBillTable monthly={bill.monthly} />
...
// ─── MONTHLY BILL TABLE ───────────────────────────────────────────────────────
function MonthlyBillTable({ monthly }) {
  ...
  }), { importKwh:0, peak:0, offpeak:0, sop:0, energyCharge:0, customerCharge:0, netBill:0 });
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ background: C.card }}>
            {['Month','Import kWh','Peak kWh','Off-Peak kWh','SOP kWh',
              'Energy $','Customer $','Total $'].map(h => (
