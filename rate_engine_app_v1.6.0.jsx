...
function drawPng(results) {
  ...
  ctx.textAlign = 'left'; ctx.fillText('MOD-04 RATE ENGINE', 24, 30);
  ctx.fillStyle = '#8b949e'; ctx.font = '11px Inter,system-ui,sans-serif';
  ctx.fillText(rateKey + (results.isCare ? ' (CARE)' : '') + '  ·  ' +
    (isBill ? 'Bill Analysis' : 'Savings Analysis') + '  ·  ' + dateStr, 24, 48);

  const cL=70, cR=W-24, cT=68, cB=H-72, cW=cR-cL, cH=cB-cT;

  if (isBill) {
    const { bill } = results;
    const hasDemand = bill.breakdown.demandCharge > 0;
    const maxVal = niceMax(Math.max(...bill.monthly.map(m => m.netBill)) * 1.18);
    const nTicks = 5;
    for (let t = 0; t <= nTicks; t++) {
      ...
      ctx.textAlign = 'right';
      ctx.fillText('$' + Math.round(v), cL - 5, y + 4);
    }
    const gW = cW / 12;
    bill.monthly.forEach((m, i) => {
      ...
      const drawSeg = (val, color) => {
        ...
        ctx.fillStyle = color;
        ctx.fillRect(x, yBot - h, bW, h);
        yBot -= h;
      };
      drawSeg(m.energyCharge,   '#58a6ff');
      if (hasDemand) drawSeg(m.demandCharge, '#f85149');
      drawSeg(m.customerCharge, '#484f58');
      ctx.fillStyle = '#e6edf3'; ctx.font = 'bold 9px Inter,system-ui,sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText('$' + Math.round(m.netBill), x + bW / 2, yBot - 3);
      ctx.fillStyle = '#8b949e'; ctx.font = '10px Inter,system-ui,sans-serif';
      ctx.fillText(MONTHS[i], x + bW / 2, cB + 14);
    });
    let lx = cL;
    ...
    legend.forEach(([lbl, col]) => {
      ctx.fillStyle = col; ctx.fillRect(lx, cB+26, 12, 10);
      ctx.fillStyle = '#8b949e'; ctx.font = '11px Inter,system-ui,sans-serif';
      ctx.fillText(lbl, lx + 16, cB + 36);
      lx += 80 + (lbl.length > 6 ? 20 : 0);
    });

  } else {
    const { baseline, solar } = results;
    const maxVal = niceMax(Math.max(...baseline.monthly.map(m => m.netBill)) * 1.18);
    const nTicks = 5;
    for (let t = 0; t <= nTicks; t++) {
      const v = maxVal * t / nTicks;
      const y = cB - (v / maxVal) * cH;
      ...
      ctx.beginPath(); ctx.moveTo(cL, y); ctx.lineTo(cR, y); ctx.stroke();
      ctx.fillStyle = '#8b949e'; ctx.font = '10px Inter,system-ui,sans-serif';
      ctx.textAlign = 'right'; ctx.fillText('$' + Math.round(v), cL - 5, y + 4);
    }
    const gW = cW / 12;
    baseline.monthly.forEach((bm, i) => {
      const sm = solar.monthly[i];
      ...
      const drawBar = (val, color, x2) => {
        ...
        ctx.fillStyle = color;
        ctx.fillRect(x2, cB - h, bW, h);
        ctx.fillStyle = '#e6edf3'; ctx.font = 'bold 9px Inter,system-ui,sans-serif';
        ctx.textAlign = 'center'; ctx.fillText('$' + Math.round(val), x2 + bW / 2, cB - h - 3);
      };
      drawBar(bm.netBill, '#d29922', bx);
      drawBar(sm.netBill, '#58a6ff', sx);
      ctx.fillStyle = '#8b949e'; ctx.font = '10px Inter,system-ui,sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText(MONTHS[i], cL + i * gW + gW / 2, cB + 14);
    });
    let lx = cL;
    [['Baseline','#d29922'],['Solar','#58a6ff']].forEach(([lbl,col]) => {
      ctx.fillStyle = col; ctx.fillRect(lx, cB+26, 12, 10);
      ctx.fillStyle = '#8b949e'; ctx.font = '11px Inter,system-ui,sans-serif';
      ctx.textAlign = 'left'; ctx.fillText(lbl, lx + 16, cB + 36);
      lx += 90;
    });
  }
  ...
  ctx.textAlign = 'right';
  ctx.fillText('MOD-04 Rate Engine v' + VERSION + '  ·  tools.cc-energy.org', W - 20, H - 12);
