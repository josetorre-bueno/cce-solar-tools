...
//     → [{ key, label, rateNote, recommendedNem, customerType, billingType, careAvailable }, ...]
//     customerType: 'residential' | 'commercial' | undefined (undefined = all rates)
//
//   MOD04.VERSION  → "1.2.0"
//   MOD04.RESIDENTIAL_RATES  (read-only, keyed rate definitions; each entry has customerType field)
//
// ─── OPTIONS (computeBill) ──────────────────────────────────────────────────────
//
...
//   - Only SDG&E bundled residential rates included in v1.2.0.
//     Commercial rates planned for v2.0.0.
...
(function (global) {
  ...
  // rateKey     string               Key into RESIDENTIAL_RATES.
  ...
  //   energyCharge, exportCredit, customerCharge, netBill
  //
  function computeBill(intervals, rateKey, options) {
    ...
    const rate = RESIDENTIAL_RATES[rateKey];
    if (!rate) {
      throw new Error(
        'Unknown rateKey: "' + rateKey + '". ' +
        'Valid keys: ' + Object.keys(RESIDENTIAL_RATES).join(', ')
      );
    }
    ...
    for (var i = 0; i < intervals.length; i++) {
      ...
      if (kwhNet > 0) {
        ...
        if (rate.billingType !== 'tiered') {
          ...
          var period   = rate.schedule(month1, hour);
    ...
    for (var j = 0; j < 12; j++) {
      var mo2 = monthly[j];
      mo2.netBill = mo2.energyCharge - mo2.exportCredit + mo2.customerCharge;
    ...
    var totalBill = totalEnergyCharge - totalExportCredit + totalCustomerCharge;
  ...
  function listRates(customerType) {
    return Object.keys(RESIDENTIAL_RATES)
      .filter(function (key) {
        var r = RESIDENTIAL_RATES[key];
  ...
  function selfTest() {
    ...
    // Test 4: listRates
    var rates = listRates();
    check('listRates count', rates.length, Object.keys(RESIDENTIAL_RATES).length, 0);
    log.push('Available rates: ' + rates.map(function(r){return r.key;}).join(', '));

    // Test 5: CARE rates — TOU-DR1 baseline should be significantly lower
    var r5 = computeBill(intervals, 'SDG&E TOU-DR1', { nemType: 'none', care: true });
    log.push('TOU-DR1 CARE annual bill: $' + r5.totalBill.toFixed(2) +
             ' (vs standard $' + r1.totalBill.toFixed(2) + ')');
    var careDiscount = ((r1.totalBill - r5.totalBill) / r1.totalBill * 100).toFixed(1);
    log.push('CARE discount: ' + careDiscount + '% reduction');
    check('CARE bill < standard bill', r5.totalBill, r5.totalBill, 0);  // runs without error
    // Test 6: CARE guard — DR should throw
